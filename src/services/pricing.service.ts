...
export const pricingService = {
  ...
  async saveClassPricing(pricing: Partial<Pricing> & { seat_class_id: number; base_rate?: number; base_price?: number }): Promise<void> {
    await dbService.init();
    ...
    if (existing) {
      dbService.run(
        ...
        [price, pricing.service_charge || 12, pricing.cgst_pct || 9, pricing.sgst_pct || 9, pricing.effective_from || '2026-01-01', pricing.effective_to || '2099-12-31', existing.id]
      );
    } else {
      dbService.run(
        ...
        [pricing.seat_class_id, price, pricing.service_charge || 12, pricing.cgst_pct || 9, pricing.sgst_pct || 9, pricing.effective_from || '2026-01-01', pricing.effective_to || '2099-12-31']
      );
    }
  },

  async saveShowPriceOverride(override: { show_id: number; seat_class_id: number; custom_base_rate?: number; base_price?: number; service_charge?: number; cgst_pct?: number; sgst_pct?: number; effective_from?: string; effective_to?: string }): Promise<void> {
    await dbService.init();
